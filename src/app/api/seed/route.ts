...
// POST /api/seed - Initialize structural data (categories, plans, settings, admin user)
// This should only be called manually or during setup, NOT automatically on page load.
export async function POST() {
  try {
    const existingChannels = await db.channel.count();
    if (existingChannels > 0) {
      return NextResponse.json({ message: 'Database already has data', channelCount: existingChannels });
    }
    ...
    if (userCount === 0) {
      ...
      await db.user.create({
        data: { email: 'admin@tgdir.com', password: DEMO_PASSWORD_HASH, name: 'Admin', username: 'admin', role: 'SUPER_ADMIN', isEmailVerified: true, isActive: true },
      });
    }

    // 5. Site Settings (only if not exists)
    ...
    return NextResponse.json({ message: 'Structural data initialized successfully' });
  } catch (error) {
...
// GET - Keep for backward compatibility but just return status
export async function GET() {
  const channelCount = await db.channel.count();
  const categoryCount = await db.category.count();
  const userCount = await db.user.count();
  return NextResponse.json({
    message: 'Seed endpoint available (POST to initialize structural data)',
    channelCount,
    categoryCount,
    userCount,
  });
